        ...
        <div class="row mb-2">
          ...
          <div class="col-sm-6">
            <ol class="breadcrumb float-sm-right">
              ...
              <li class="breadcrumb-item active">@lang('site.bloodTypes_list')</li>
            </ol>
          </div><!-- /.col -->
        </div><!-- /.row -->
      </div><!-- /.container-fluid -->
        ...
        <div class="col-12">
          <div class="card">
            <div class="card-header">
              <h3 class="card-title">@lang('site.bloodTypes_list')</h3>
            </div>
            <!-- /.card-header -->
            <div class="card-body">
            @if($bloodTypes->count() > 0)
              <table id="example2" class="table table-bordered table-hover">
                ...
                @foreach($bloodTypes as $index=>$bloodType)
                    <tr id="removable{{$bloodType->id}}">
                        <td>{{ $index + 1 }}</td>
                        <td>{{ $bloodType->name }}</td>
                        <td>
                        @if(auth()->user()->hasPermission('update_bloodTypes'))
                        <a href="{{ route('dashboard.bloodTypes.edit',$bloodType->id)}}"class="btn btn-info btn-sm"><i class="fa fa-edit"></i> @lang('site.edit')</a>
                        @endif

                        @if(auth()->user()->hasPermission('delete_bloodTypes'))
                        <button id="{{$bloodType->id}}" data-token="{{ csrf_token() }}"
                            data-route="{{URL::route('dashboard.bloodTypes.destroy',$bloodType->id)}}"
